...
def is_terminal_or_vscode_focused():
    ...
    if any(title in active_window_title for title in terminal_titles) or vscode_title in active_window_title == "NoneType":
        return vscode_title
...
def play_music_in_order(directory, order=None, loop=True):
    ...
    paused = False
    last_pressed_time = 0
    delay = 0.2  # Adjust the delay time as needed
    ...
    while playing:
        ...
        if is_terminal_or_vscode_focused() and (keyboard.is_pressed('P') or keyboard.is_pressed('K')) and time.time() - last_pressed_time > delay:
            if not paused:
        ...
        # Check for key press to skip forward 5 seconds if terminal or vscode is focused
        if is_terminal_or_vscode_focused() and keyboard.is_pressed('l') and time.time() - last_pressed_time > delay:
            current_position = pygame.mixer.music.get_pos()
            new_position = current_position + 5000  # 5000 milliseconds = 5 seconds
            pygame.mixer.music.set_pos(new_position / 1000)  # Set position in seconds
            print("Skipped forward 5 seconds")
            last_pressed_time = time.time()

        # Check for key press to go back 5 seconds if terminal or vscode is focused
        if is_terminal_or_vscode_focused() and keyboard.is_pressed('j') and time.time() - last_pressed_time > delay:
            current_position = pygame.mixer.music.get_pos()
            new_position = max(0, current_position - 5000)  # Ensure new position is not negative
            pygame.mixer.music.set_pos(new_position / 1000)  # Set position in seconds
...
if __name__ == "__main__":
    ...
    order = ["MASHLE MAGIC AND MUSCLES The Divine Visionary Candidate Exam Arc  OPENING THEME - 128.mp3",
            ...
            "JoJo's Bizarre Adventure Golden Wind OST - Giorno's Theme『il vento d'oro』 - 128.mp3"]
